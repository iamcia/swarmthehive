...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_status'])) {
    ...
    $updateSql = "UPDATE ownertenantmoveout SET Status = ? WHERE Resident_Code = ?";
    $updateStmt = $conn->prepare($updateSql);
    $updateStmt->bind_param('ss', $status, $residentCode);

    $_SESSION['message'] = $updateStmt->execute() ? "Status updated successfully!" : "Error updating status: " . $updateStmt->error;
    $updateStmt->close();
    header("Location: OwnerTenantMoveOut.php"); 
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_record'])) {
    ...
    $deleteSql = "DELETE FROM ownertenantmoveout WHERE Resident_Code = ?";
    $deleteStmt = $conn->prepare($deleteSql);
    $deleteStmt->bind_param('s', $residentCodeToDelete);

    $_SESSION['message'] = $deleteStmt->execute() ? "Record deleted successfully!" : "Error deleting record: " . $deleteStmt->error;
    $deleteStmt->close();
    header("Location: OwnerTenantMoveOut.php"); 
    exit;
}
...
$sql = "SELECT * FROM ownertenantmoveout WHERE 1=1";
$params = [];
$types = "";
...
<main>
    <h2>Owner/Tenant Move-Out List</h2>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['parkingSlotNumber']); ?></td>
                            <td><?php echo htmlspecialchars($row['days_prior_moveout']); ?></td>
                            <td><?php echo htmlspecialchars($row['representativeName']); ?></td>
                            <td><?php echo htmlspecialchars($row['Resident_Contact']); ?></td>
                            <td>
                                <?php if (!empty($row['Signature'])): ?>
                                    <img src="data:image/png;base64,<?php echo base64_encode($row['Signature']); ?>" alt="Signature" width="50">
                                <?php else: ?>
                                    No Signature
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="action-buttons">
                                ...
                                <form method="POST" style="display:inline;">
                                    <input type="hidden" name="resident_code_to_delete" value="<?php echo $row['Resident_Code']; ?>">
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="10">No move-out records found</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
