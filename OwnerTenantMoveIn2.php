...
$sql = "SELECT Resident_Code, currentDate, Resident_Name, parkingSlotNumber, leaseExpiryDate, representativeName, Resident_Contact, Signature, Status, created_at FROM ownertenantmovein WHERE Status = 'Pending'";
...
<style>
       body {
        ...
        .card {
            ...
            background-color: #52BE80; /* Set the background color */
...
<main>
    <div class="header-container">
        ...
        <button class="redirect-btn" onclick="window.location.href='OwnerTenantMoveIn.php'">Go To Table</button>
    </div>
    <div class="card-container">
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="card" onclick="showForm('<?php echo $row['Resident_Code']; ?>', '<?php echo $row['currentDate']; ?>', '<?php echo $row['Resident_Name']; ?>', '<?php echo $row['parkingSlotNumber']; ?>', '<?php echo $row['leaseExpiryDate']; ?>', '<?php echo $row['representativeName']; ?>', '<?php echo $row['Resident_Contact']; ?>', '<?php echo $row['Signature']; ?>', '<?php echo $row['Status']; ?>', '<?php echo $row['created_at']; ?>')">
    ...
    <div id="form-container" class="hidden-form">
        ...
        <div class="movein-details">
            ...
            <p><strong>Lease Expiry:</strong> <span id="form-lease-expiry"></span></p>
...
<script>
function showForm(residentCode, date, residentName, parkingSlot, leaseExpiry, representative, contact, signature, status, createdAt) {
    ...
    document.getElementById('form-lease-expiry').textContent = leaseExpiry;
...
function exportPDF() {
    ...
    let content = `
        ...
        Lease Expiry: ${document.getElementById('form-lease-expiry').textContent}
