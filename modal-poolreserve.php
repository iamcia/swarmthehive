...
$query = "
    ...
    JOIN 
        poolreserve pr ON sr.service_id = pr.id
    WHERE 
        sr.id = $request_id AND sr.service_type = 'PoolReserve'
";
...
// Parse names JSON data
$names = [];
if (!empty($request['names'])) {
    $names = json_decode($request['names'], true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        // If not valid JSON, treat as regular text
        $names = [['first_name' => $request['names'], 'last_name' => '']];
    }
}

// Format schedule (could be date, time, or date + time)
function formatSchedule($scheduleStr) {
    if (empty($scheduleStr)) return 'N/A';
    
    // Check if it contains date and time
    if (strpos($scheduleStr, ' ') !== false) {
        // Try to parse as datetime
        try {
            $datetime = new DateTime($scheduleStr);
            return $datetime->format('F j, Y \a\t h:i A');
        } catch (Exception $e) {
            return $scheduleStr; // Return as is if parsing fails
        }
    }
    
    // If it's just a date
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $scheduleStr)) {
        return formatDate($scheduleStr);
    }
...
<div class="flex justify-between items-center p-4 border-b border-gray-200">
    <h2 class="text-xl font-medium flex items-center gap-2">
        Pool Reservation <span class="text-gray-500 font-normal">#<?php echo $request['request_id']; ?></span>
    </h2>
...
<div class="p-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <p class="text-gray-800">
                        <?php 
                        if (!empty($request['towerunitnum'])) {
                            echo $request['towerunitnum'];
                        } else {
                            echo "Tower " . $request['Tower'] . ", Unit " . $request['Unit_Number'];
                        }
                        ?>
                    </p>
                </div>
        ...
        <div class="bg-gray-50 p-5 rounded-lg">
            <h3 class="text-lg font-medium text-gray-800 border-b border-gray-200 pb-2 mb-4">
                <i class="fas fa-swimming-pool mr-2 text-blue-primary"></i>Pool Reservation Details
            </h3>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1">Schedule:</label>
                    <p class="text-gray-800 font-medium"><?php echo formatSchedule($request['schedule']); ?></p>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1">Number of Swimmers:</label>
                    <p class="text-gray-800"><?php echo count($names); ?> person<?php echo count($names) > 1 ? 's' : ''; ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6">
        <h3 class="text-lg font-medium text-gray-800 border-b border-gray-100 pb-2 mb-4">
            <i class="fas fa-users mr-2 text-blue-primary"></i>Listed Person
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <?php foreach($names as $index => $person): ?>
                <div class="p-4 bg-gray-50 rounded-lg">
                    <h4 class="font-medium text-gray-700 mb-2">Person <?php echo $index + 1; ?></h4>
                    <div class="grid grid-cols-2 gap-2">
                        <?php if (isset($person['first_name'])): ?>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">First Name:</label>
                            <p class="text-gray-800"><?php echo $person['first_name']; ?></p>
                        </div>
                        <?php endif; ?>
                        
                        <?php if (isset($person['last_name'])): ?>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Last Name:</label>
                            <p class="text-gray-800"><?php echo $person['last_name']; ?></p>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
...
<script>
    ...
    function updateStatus(requestId, status, rejectReason = '') {
        ...
        data.append('service_type', 'PoolReserve');
