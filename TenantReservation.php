...
if (isset($_SESSION['username'])) {
    ...
    if ($result_owner->num_rows > 0) {
        ...
        $tenantId = null; // Owners do not have Tenant_ID
    } else {
        ...
        if ($result_tenant->num_rows > 0) {
            ...
            $tenantId = $row_tenant['Tenant_ID']; // Tenant ID for foreign key reference
        }
        $stmt_tenant->close();
    }
    $stmt_owner->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_reservation'])) {
    ...
    $catererName = isset($_POST['caterer_name']) && !empty($_POST['caterer_name']) ? htmlspecialchars($_POST['caterer_name']) : NULL;
    $cateringContact = isset($_POST['catering_contact']) && !empty($_POST['catering_contact']) ? htmlspecialchars($_POST['catering_contact']) : NULL;
...
$stmt->bind_param("ssssssisssssi", 
    ...
    $cateringContact,
    $status,
    $tenantId // Use the Tenant_ID here
);
...
<body class="bg-gray-50 min-h-screen">
    <div class="max-w-4xl mx-auto px-4 py-10">
       ...
       <!-- Form Container -->
    <form method="POST" action="TenantReservation.php" class="space-y-8">
        <!-- Hidden Fields -->
<input type="hidden" id="tenant_id" name="tenant_id" value="<?php echo htmlspecialchars($tenantId); ?>" readonly>
