...
if (isset($_POST['delete'])) {
    $owner_id = $_POST['delete'];
    $delete_sql = "DELETE FROM ownerinformation WHERE Owner_ID = ?";
...
if (isset($_POST['approve']) || isset($_POST['disapprove'])) {
    $owner_id = isset($_POST['approve']) ? $_POST['approve'] : $_POST['disapprove'];
    $status = isset($_POST['approve']) ? "Approved" : "Disapproved";
    $update_sql = "UPDATE ownerinformation SET Status = ? WHERE Owner_ID = ?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("ss", $status, $owner_id);
    $stmt->execute();
    $stmt->close();
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
...
if (isset($_POST['add'])) {
    ...
    $select_sql = "SELECT * FROM ownerinformation WHERE Access_Code IS NOT NULL AND Tower IS NULL AND Unit_Number IS NULL LIMIT 1";
    ...
    if ($result->num_rows > 0) {
        ...
        $update_sql = "UPDATE ownerinformation SET 
                        Tower = ?, Unit_Number = ?, Last_Name = ?, First_Name = ?, Middle_Name = ?, 
                        Mobile_Number = ?, Home_Number = ?, Month_Issue = ?, Year_Issue = ?, 
                        Username = ?, Password = ?, Government_ID = ?, Proof_Residency = ?, Signature = ?, Status = ?, Owner_ID = ?
        ...
        // Insert a new record
        $insert_sql = "INSERT INTO ownerinformation (Tower, Unit_Number, Last_Name, First_Name, Middle_Name, 
                        Mobile_Number, Home_Number, Month_Issue, Year_Issue, Username, Password, Government_ID, 
                        Proof_Residency, Signature, Status, Owner_ID, Access_Code, Email) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL)";
...
$sql = "SELECT * FROM ownerinformation";
...
$sql = "SELECT * FROM ownerinformation WHERE 1=1";
...
.search-filter-container button[type="submit"]:hover {
    background-color: #0056b3;
        ...
        <script>
        function showForm(tower, unitNumber, monthIssue, yearIssue, lastName, firstName, middleName, mobileNumber, homeNumber, username, password) {
            ...
            document.getElementById('monthIssue').value = monthIssue || '';
            document.getElementById('yearIssue').value = yearIssue || '';
            document.getElementById('lastName').value = lastName || '';
            document.getElementById('firstName').value = firstName || '';
            document.getElementById('middleName').value = middleName || '';
            document.getElementById('mobileNumber').value = mobileNumber || '';
            document.getElementById('homeNumber').value = homeNumber || '';
        ...
        // Function to copy the email entered in the first form to the second form
        function copyEmail() {
            var email = document.getElementById("email").value;
            document.getElementById("emails").value = email; // Set the value of the second email input
        }

        // Function to show the alert box with a message
        function showMessage(message) {
            if (message !== '') {
                alert(message); // Show the alert message if there's a message
            }
        }
...
<main>
                ...
                <?php while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>
                            <form method='POST' style='display:inline;'>
                                <button type='submit' name='delete' value='" . htmlspecialchars($row['Owner_ID']) . "'>Delete</button>
                                <button type='submit' name='approve' value='" . htmlspecialchars($row['Owner_ID']) . "'>Approve</button>
                                <button type='submit' name='disapprove' value='" . htmlspecialchars($row['Owner_ID']) . "'>Disapprove</button>
                            </form>
                          </td>";
                    echo "</tr>";
                } ?>
    ...
    <form id="addForm" method="POST" enctype="multipart/form-data" style="display: none;">
        ...
        <div class="form-group">
            <button type="submit" name="add">Add Owner</button>
        </div>
    </form>
    ...
    <div class="container">
        ...
        <div class="generate-form">
            <form method="POST">
                ...
                <div>
                    <input type="email" id="email" name="email" value="<?php echo $email; ?>" oninput="copyEmail()"> <!-- Email field value persists -->
                    </div>
                <div>
                    <button type="submit" name="saveOwner">Save In Owner Table</button>
                </div>
            </form>
        </div>
    <div class="container">
