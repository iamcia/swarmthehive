...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($names as $guest) {
        ...
        if ($count > 0) {
            ...
            echo "<script>
                    alert('$errorMessage');
                    window.location.href='ReservePool.php';
                  </script>";
            exit();
        }
    }
...
<body class="bg-gradient-to-br from-blue-50 to-gray-100 min-h-screen font-sans flex flex-col items-center p-5 md:p-10">
    ...
    <div class="w-full max-w-2xl bg-white rounded-xl shadow-custom-lg overflow-hidden">
        ...
        <!-- Form Content -->
        <form id="poolReservationForm" method="POST" action="Insert-ReservePool.php" enctype="multipart/form-data" class="p-6 md:p-8 space-y-6">
            ...
            <div class="space-y-3">
                ...
                <p class="text-xs text-gray-500 italic">Please add all guests who will be using the pool facility. Maximum of 5 guests allowed.</p>
            </div>
    ...
    <script>
        let guestCount = 1; // Start with 1 guest

        // Function to add a guest
        function addNameEntry(button) {
            if (guestCount >= 5) {
                alert("You can only add a maximum of 5 guests.");
                return;
            }
