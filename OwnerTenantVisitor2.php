...
$sql = "SELECT Resident_Code, user_type, start_date, end_date, guest_info, valid_id, signature, Status FROM ownertenantvisitor WHERE Status = 'Pending'";
...
<style>
       body {
        ...
        .card {
            ...
            background-color: #AF7AC5; /* Set the background color */
...
<main>
    <div class="header-container">
        ...
        <button class="redirect-btn" onclick="window.location.href='OwnerTenantVisitor.php'">Go To Table</button>
    </div>

    <div class="card-container">
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <?php $guest_info = json_decode($row['guest_info'], true);
                      $formattedGuests = $guest_info ? implode(", ", $guest_info) : 'N/A'; ?>
                <div class="card" onclick="showForm(
                    '<?= htmlspecialchars($row['Resident_Code']) ?>',
                    '<?= htmlspecialchars($row['user_type']) ?>',
                    '<?= htmlspecialchars($formattedGuests) ?>',
                    ...
                    <p>Start Date: <?= htmlspecialchars($row['start_date']) ?></p>
                    <p>End Date: <?= htmlspecialchars($row['end_date']) ?></p>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
...
<div id="form-container" class="hidden-form">
    ...
    <div class="reservation-details">
        ...
        <p><strong>Guest Info:</strong> <span id="form-guest-info"></span></p>
        <p><strong>Start Date:</strong> <span id="form-start-date"></span></p>
        <p><strong>End Date:</strong> <span id="form-end-date"></span></p>
...
<script>
function showForm(residentCode, userType, guestInfo, startDate, endDate, status) {
    const fields = {
        'form-resident-code': residentCode,
        'form-user-type': userType,
        'form-guest-info': guestInfo,
...
function exportVisitorRequestToPDF() {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();

    doc.text("Visitor Request Form", 10, 10);
    ...
    addText("Guest Info", document.getElementById('form-guest-info').textContent);
    addText("Start Date", document.getElementById('form-start-date').textContent);
    addText("End Date", document.getElementById('form-end-date').textContent);
    addText("Status", document.getElementById('form-status').textContent);

    doc.save("Visitor_Request.pdf");
}
</script>
